...
namespace ThePixelGamer\DrawMyThing;
						...
						
					{
						for($i=1;$i<=8;$i++)
						{
							if($this->arena->get($i . "Name") == $player->getName())
							{
								...
								{
									$player->sendMessage(TextFormat::GREEN . "§7You're the first!§r\n§eYou got +3 points!§r");
									$this->arena->set($i . "Points", $this->arena->get($i . "Points")+3);
								}
								else
								{
									$player->sendMessage(TextFormat::GREEN . "Correct!\n You got 1 point!§r");
								...
								{
									$inp->sendMessage(TextFormat::GREEN . "§3DMT §7>>§e " . $player->getName() . " §7has guessed the word!§r");
									$aop++;
								}
								}
		...
		else if($event->getPlayer()->getLevel()->getTile($event->getBlock()) instanceOf Sign)
		{
			...
			if($text[0]=="Draw my Thing" || $text[0]== TextFormat::BLUE . "Draw my Thing")
			{
				...
				if($this->arena->get("ingame")==false)
				{
					...
					{
						$player->sendPopup(TextFormat::RED . "This game is full");
					}
				}
				else
				{
					$player->sendPopup(TextFormat::RED . "This game is running");
				}
			}
		}
		...
		foreach($arenas as $a)
		{
			...
			if($playersinarena==1)
			{
				foreach($allplayers as $player)
				{
					if($player->getLevel()->getFolderName()==$arenaname)
					{
						$player->sendPopup(TextFormat::RED . "Not enough players yet");
					}
				}
			...
			else if($playersinarena>=2)
			{
				...
				if($time>=1&&$this->arena->get("ingame")==false)
				{
					foreach($allplayers as $player)
					{
						if($player->getLevel()->getFolderName()==$arenaname)
						{
							$player->sendPopup(TextFormat::YELLOW . $time . " seconds");
						}
					}
					$time -= 1;
				...
				else
				{
					...
					else
					{
						foreach($allplayers as $player)
						{
							if($player->getLevel()->getFolderName()==$arenaname)
							{
								if($player->getAllowFlight()==false)
								{
									...
									$player->sendPopup(TextFormat::BOLD . $newword."§r");
								}
							}
						}
						...
						foreach($allplayers as $player)
						{
							if($player->getLevel()->getFolderName()==$arenaname)
							{
								$player->sendMessage(TextFormat::GOLD. "§e" . $bt . "§7 second§8(§7s§8) §7remaining!§r");
							}
						}
						}
						...
						foreach($allplayers as $player)
						{
							if($player->getLevel()->getFolderName()==$arenaname)
							{
								$player->sendMessage(TextFormat::GOLD . ($bt/60) . "§7 minute§8(§7s§8) §7remaining!§r");
							}
						}
						}
